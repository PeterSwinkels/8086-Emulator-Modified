...
Public Module InterruptHandlerModule
   ...
   Private Const sHW_FILE As String = "C:\emu8086.hw"   'Defines the emu8086 hardware interrupt file.

   'This procedure executes hardware interrupts.
   Public Sub ExecuteHardwareInterrupts()
      Try
         Dim Interrupts() As Byte = {}

         If File.Exists(sHW_FILE) Then
            Interrupts = File.ReadAllBytes(sHW_FILE)

            For Vector As Integer = Interrupts.GetLowerBound(0) To Interrupts.GetUpperBound(0)
               If Not Interrupts(Vector) = &H0% Then
                  CPU.ExecuteInterrupt(OpcodesE.INT, Vector)
                  Interrupts(Vector) = &H0%
               End If
            Next Vector

            File.WriteAllBytes(sHW_FILE, Interrupts)
         End If
